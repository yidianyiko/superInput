...
static int whisper_model_mel_count(const whisper_context * ctx) {
    if (ctx == nullptr) {
        return WHISPER_N_MEL;
    }

    if (ctx->model.filters.n_mel > 0) {
        return ctx->model.filters.n_mel;
    }

    if (ctx->model.hparams.n_mels > 0) {
...
int whisper_pcm_to_mel_with_state(struct whisper_context * ctx, struct whisper_state * state, const float * samples, int n_samples, int n_threads) {
    const int n_mel = whisper_model_mel_count(ctx);

    if (!log_mel_spectrogram(*state, samples, n_samples, WHISPER_SAMPLE_RATE, WHISPER_N_FFT, WHISPER_HOP_LENGTH, n_mel, n_threads, ctx->model.filters, false, state->mel)) {
...
int whisper_pcm_to_mel_phase_vocoder_with_state(struct whisper_context * ctx, struct whisper_state * state, const float * samples, int n_samples, int n_threads) {
    ...
    if (!log_mel_spectrogram(*state, samples, n_samples, WHISPER_SAMPLE_RATE, 2 * WHISPER_N_FFT, 2 * WHISPER_HOP_LENGTH, n_mel, n_threads, ctx->model.filters, true, state->mel)) {
        fprintf(stderr, "%s: failed to compute mel spectrogram\n", __func__);
        return -1;
    }
        ...
        struct whisper_context * ctx,
          struct whisper_state * state,
                   const float * data,
                           int   n_len,
                           int   n_mel) {
    const int expected_n_mel = whisper_model_mel_count(ctx);

    if (n_mel != expected_n_mel) {
        fprintf(stderr, "%s: invalid number of mel bands: %d (expected %d)\n", __func__, n_mel, expected_n_mel);
        return -1;
    }
